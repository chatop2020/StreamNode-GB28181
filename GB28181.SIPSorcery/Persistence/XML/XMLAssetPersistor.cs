...
namespace GB28181.Persistence.XML
{
    public class XMLAssetPersistor<T> : SIPAssetPersistor<T> where T : ISIPAsset, new()
    {
        ...
        public XMLAssetPersistor(string xmlFilePath)
        {
            ...
            {
                Logger.Logger.Warn(
                    "File " + m_xmlAssetFilePath + " does not exist for SIPAssetXMLPersistor, creating new one.");
        ...
        public override T Update(T sipAsset)
        {
            try
            {
                ...
                {
                    throw new ApplicationException("Could not update SIP Asset with id " + sipAsset.Id + " (for " +
                                                   typeof(T).Name + "), no existing asset found.");
                }
            }
            catch (Exception excp)
            {
                Logger.Logger.Error("Exception SIPAssetXMLPersistor Update. ->" + excp.Message);
                throw;
            }
        }
        ...
        public override void UpdateProperty(Guid id, string propertyName, object value)
        {
            try
            {
                ...
                if (property == null)
                {
                    throw new ApplicationException("Property " + propertyName + " for " + typeof(T).Name +
                                                   " could not be found, UpdateProperty failed.");
                }
                else
                {
                    if (m_sipAssets.ContainsKey(id))
            ...
            {
                Logger.Logger.Error("Exception SIPAssetXMLPersistor UpdateProperty (for " + typeof(T).Name + "). ->" +
                                    excp.Message);
                throw;
            }
        }
        ...
        public override void IncrementProperty(Guid id, string propertyName)
        {
            try
            {
                ...
                {
                    throw new ApplicationException("Property " + propertyName + " for " + typeof(T).Name +
                                                   " could not be found, IncrementProperty failed.");
                }
                else
                {
                    if (m_sipAssets.ContainsKey(id))
                    {
                        lock (m_sipAssets)
                        {
                            property.SetValue(m_sipAssets[id],
                                Convert.ToInt32(property.GetValue(m_sipAssets[id], null)) + 1, null);
                        }
        ...
        public override void DecrementProperty(Guid id, string propertyName)
        {
            try
            {
                ...
                {
                    throw new ApplicationException("Property " + propertyName + " for " + typeof(T).Name +
                                                   " could not be found, DecrementProperty failed.");
                }
                else
                {
                    if (m_sipAssets.ContainsKey(id))
        ...
        public override void Delete(T sipAsset)
        {
            try
            {
                ...
                Logger.Logger.Debug("SIPAssetsXMLPersistor attempting to delete " + sipAsset.Id + " type " +
                                    sipAsset.GetType().ToString() + ".");
                ...
                {
                    throw new ApplicationException("An attempt was made to delete a non-existent " +
                                                   sipAsset.GetType().ToString() + " failed in SIPAssetXMLPersistor.");
                }
            }
            catch (ApplicationException appExcp)
            {
                Logger.Logger.Error(appExcp.Message);
            }
            catch (Exception excp)
            {
                Logger.Logger.Error("Exception Delete (for " + typeof(T).Name + "). ->" + excp.Message);
                throw;
            }
        }
        ...
        public override void Delete(Expression<Func<T, bool>> whereClause)
        {
            ...
            {
                Logger.Logger.Error("Exception SIPAssetXMLPersistor Delete (batch) (for " + typeof(T).Name + "). ->" +
                                    excp.Message);
            }
        }
        ...
        public override List<T> Get()
        {
            ...
            else
            {
                ...
                {
                    Logger.Logger.Debug("Could not locate a " + typeof(T).Name);
                    return new List<T>();
                }
            }
        }
        ...
        public override object GetProperty(Guid id, string propertyName)
        {
            ...
            else
            {
                if (m_sipAssets.ContainsKey(id))
                {
                    ...
                    {
                        throw new ApplicationException("Property " + propertyName + " for " + typeof(T).Name +
                                                       " could not be found, GetProperty failed.");
                    }
                    else
                    {
        ...
        private void LoadSIPAssetsFromXML(XmlDocument xmlDom)
        {
            ...
            if (m_xmlFileWatcher == null)
            {
                ...
                Logger.Logger.Debug("Starting file watch on " + dir + "\\" + file + ".");
        ...
        private void AssetXMLFileChanged(object sender, FileSystemEventArgs e)
        {
            if (DateTime.Now.Subtract(m_lastReload).TotalSeconds > RELOAD_SPACING_SECONDS)
            {
                try
                {
                    ...
                    Logger.Logger.Debug("Reloading SIP Assets.");
                ...
                {
                    Logger.Logger.Error("Exception AssetXMLFileChanged. ->" + excp.Message);
                }
            }
        }
        ...
        private void WriteSIPAssetXMLAsync(object state)
        {
            ...
            {
                Logger.Logger.Error("Exception WriteSIPAssetXMLAsync. ->" + excp.Message);
            }
            finally
            {
                m_savePending = false;
                ...
                {
                    Logger.Logger.Error("Exception WriteSIPAssetXMLAsync finally. ->" + excp.Message);
                }
            }
        }
        ...
        public static Dictionary<Guid, object> LoadAssetsFromXMLRecordSet(XmlDocument dom)
        {
            try
            {
                    ...
                    foreach (DataRow row in sipAssetSet.Tables[0].Rows)
                    {
                        ...
                        catch (Exception excp)
                        {
                            Logger.Logger.Error("Exception loading SIP asset record in LoadAssetsFromXMLRecordSet (" +
                                                (new T()).GetType().ToString() + "). ->" + excp.Message);
                        }
                    }

                    Logger.Logger.Debug(assets.Count + " " + (new T()).GetType().ToString() +
                ...
                else
                {
                    ...
                    Logger.Logger.Debug("No" + (new T()).GetType().ToString() + " assets loaded from XML record set.");
                }
