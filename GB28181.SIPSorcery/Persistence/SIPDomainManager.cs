...
namespace GB28181.App
{
    public delegate string
        GetCanonicalDomainDelegate(string host,
            bool wildCardOk); // Used to get the canonical domain from a host portion of a SIP URI.
    ...
    public class SIPDomainManager
    {
        ...
        private static Dictionary<string, SIPDomain>
            m_domains = new Dictionary<string, SIPDomain>(); // Records the domains that are being maintained.
            ...
            try
            {
                ...
                if (sipDomainList == null || sipDomainList.Count == 0)
                {
                    throw new ApplicationException(
                        "No SIP domains could be loaded from the persistence store. There needs to be at least one domain.");
                }
                else
                {
                    m_domains.Clear();
        ...
        private SIPDomain GetSIPDomain(string host, bool wildcardOk)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var targetDomain = allAliases?.FirstOrDefault(domainItem =>
                        domainItem.Aliases.Exists(aliasItem => aliasItem.ToLower() == host.ToLower()));
