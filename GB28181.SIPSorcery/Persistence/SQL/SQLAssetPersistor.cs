...
namespace GB28181.Persistence
{
    public class SQLAssetPersistor<T> : SIPAssetPersistor<T> where T : class, ISIPAsset, new()
    {
            ...
            using (IDbTransaction trans = connection.BeginTransaction())
            {
                try
                {
                    ...
                    foreach (var asset in assets)
                    {
                        ...
                        {
                            DbParameter dbParameter = base.GetParameter(m_dbProviderFactory, propertyValue.Key,
                                propertyValue.Value, paramNumber.ToString());
                        ...
                        string insertCommandText = insertQuery.ToString().TrimEnd(',') + ") values " +
                                                   parametersStr.ToString().TrimEnd(',') + ")";
                ...
                {
                    trans.Rollback();
                    Logger.Logger.Error("Exception SQLAssetPersistor Add (for " + typeof(T).Name + "). ->" +
                                        excp.Message);
                    throw;
                }
            }
        ...
        public override T Update(T asset)
        {
            ...
            {
                Logger.Logger.Error(
                    "Exception SQLAssetPersistor Update (for " + typeof(T).Name + "). ->" + excp.Message);
                throw;
            }
                ...
                using (IDbTransaction trans = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        deleteCommand.CommandText =
                            "delete from " + m_objectMapper.TableName + " where id = '" + asset.Id + "'";
                        deleteCommand.ExecuteNonQuery();
                        trans.Commit();
                        //   Deleted?.(asset);
                    }
        ...
        public override T Get(Guid id)
        {
            ...
            {
                Logger.Logger.Error("Exception SQLAssetPersistor Get (id) (for " + typeof(T).Name + "). ->" +
                                    excp.Message);
                throw;
            }
        }
        ...
        public override object GetProperty(Guid id, string propertyName)
        {
            try
            {
                using (IDbConnection connection = m_dbProviderFactory.CreateConnection())
                {
                    ...
                    IDbCommand command = connection.CreateCommand();
                    command.CommandText = "select " + propertyName + " from " + m_objectMapper.TableName +
                                          " where id = '" + id + "'";
            ...
            {
                Logger.Logger.Error("Exception SQLAssetPersistor GetProperty (for " + typeof(T).Name + "). ->" +
                                    excp.Message);
                throw;
            }
        }
        ...
        public override int Count(Expression<Func<T, bool>> whereClause)
        {
            ...
            {
                Logger.Logger.Error("Exception SQLAssetPersistor Count (for " + typeof(T).Name + "). ->" +
                                    excp.Message);
                throw;
            }
        }
        ...
        public override T Get(Expression<Func<T, bool>> whereClause)
        {
            ...
            catch (Exception excp)
            {
                ...
                Logger.Logger.Error("Exception SQLAssetPersistor Get (where) (for " + typeof(T).Name + "). ->" +
                                    whereClauseStr +
                                    excp);
                throw;
            }
