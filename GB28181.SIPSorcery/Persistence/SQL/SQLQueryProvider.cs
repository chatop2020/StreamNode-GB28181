    ...
    public class SQLQueryProvider : QueryProvider
    {
        ...
        public override object Execute(Expression expression)
        {
            try
            {
                ...
                bool isIQueryable = (expression.Type.FullName.StartsWith("System.Linq.IQueryable") ||
                                     expression.Type.FullName.StartsWith("System.Linq.IOrderedQueryable"));
                ...
                else
                {
                    throw new ApplicationException(
                        "The expression translation by the SQLQueryProvider resulted in an empty select string.");
                }
            }
        ...
        public class SQLQueryProviderUnitTest {
            ...
            private class MockSIPAsset : ISIPAsset {
                ...
                private Guid m_id;
                [Column(Storage = "m_id", Name = "id", DbType = "character varying(36)", IsPrimaryKey = true, CanBeNull
 = false)]
                public Guid Id {
                    get { return m_id; }
                    set { m_id = value; }
            ...
            public void SelectWithAndOperatorTest() {
                ...
                SQLQueryProvider queryProvider = new SQLQueryProvider(null, null, null, null);
                Expression<Func<MockSIPAsset, bool>> whereClause =
 (asset) => asset.AdminId == "1234" && asset.Username == "abcd";
                string querytext = queryProvider.GetQueryText(whereClause);
                Console.WriteLine("Query: " + querytext);
            ...
            public void SelectWithNotCombinedWithAndOperatorTest() {
                ...
                DateTime checkDate = DateTime.Now;
                Expression<Func<MockSIPAsset, bool>> whereClause =
 (asset) => !asset.Expired && asset.Inserted >= checkDate;
                string querytext = queryProvider.GetQueryText(whereClause);
                Console.WriteLine("Query: " + querytext + ".");
            ...
            public void SelectWithMemberVariableCombinedWithAndClauseOperatorTest() {
                ...
                DateTime checkDate = DateTime.Now;
                Expression<Func<MockSIPAsset, bool>> whereClause =
 (asset) => asset.Username == myObj.Name && !asset.Expired && asset.Inserted >= checkDate;
                string querytext = queryProvider.GetQueryText(whereClause);
                Console.WriteLine("Query: " + querytext);
