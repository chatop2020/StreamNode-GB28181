    ...
    public class SIPTransport : ISIPTransport
    {
        ...
        private const int
            MAX_QUEUEWAIT_PERIOD =
                2000; // Maximum time to wait to check the message received queue if no events are received.

        private const int
            PENDINGREQUESTS_CHECK_PERIOD =
                500; // Time between checking the pending requests queue to resend reliable requests that have not been responded to.
        ...
        private const int
            MAX_RELIABLETRANSMISSIONS_COUNT =
                5000; // The maximum number of messages that can be maintained for reliable transmissions.

        public const string ALLOWED_SIP_METHODS =
            "ACK, BYE, CANCEL, INFO, INVITE, NOTIFY, OPTIONS, REFER, REGISTER, SUBSCRIBE";
       ...
       // private static ILog logger = AssemblyState.logger;
        ...
        private bool
            m_queueIncoming =
                true; // Dictates whether the transport later will queue incoming requests for processing on a separate thread of process immediately on the same thread.
        ...
        private Dictionary<string, SIPChannel>
            m_sipChannels =
                new Dictionary<string, SIPChannel>(); // List of the physical channels that have been opened and are under management by this instance.
        ...
        public IPAddress
            ContactIPAddress; // If set this address will be passed to the UAS Invite transaction so it can be used as the Contact address in Ok responses.
        ...
        private bool
            m_reliablesThreadRunning = false; // Only gets started when a request is made to send a reliable request.
            ...
            bool queueIncoming)
        {
            if (sipResolver == null)
            {
                throw new ArgumentNullException(
                    "The SIP end point resolver must be set when creating a SIPTransport object.");
            }
        ...
        public void SendRaw(SIPEndPoint localSIPEndPoint, SIPEndPoint destinationEndPoint, byte[] buffer)
        {
            ...
            if (m_sipChannels.Count == 0)
            {
                throw new ApplicationException(
                    "No channels are configured in the SIP transport layer. The data could not be sent.");
            }
        ...
        public void SendRequest(SIPRequest sipRequest)
        {
            ...
            {
                throw new ApplicationException(
                    "No channels are configured in the SIP transport layer. The request could not be sent.");
            }
            ...
            {
                SIPResponse unresolvableResponse = GetResponse(sipRequest, SIPResponseStatusCodesEnum.AddressIncomplete,
                    "DNS resolution for " + dnsResult.URI.Host + " failed " + dnsResult.LookupError);
        ...
        public void SendRequest(SIPEndPoint dstEndPoint, SIPRequest sipRequest)
        {
            ...
            {
                throw new ApplicationException("A default SIP channel could not be found for protocol " +
                                               sipRequest.LocalSIPEndPoint.Protocol + " when sending SIP request.");
            }
        }
        ...
        private void SendRequest(SIPChannel sipChannel, SIPEndPoint dstEndPoint, SIPRequest sipRequest)
        {
            try
            {
                ...
                sipRequest.Header.ContentLength = (sipRequest.Body.NotNullOrBlank())
                    ? Encoding.UTF8.GetByteCount(sipRequest.Body)
                    : 0;
            ...
            catch (ApplicationException appExcp)
            {
                ...
                {
                    if (IsLocalSIPEndPoint(
                        SIPEndPoint.ParseSIPEndPoint(errorResponse.Header.Vias.TopViaHeader.ReceivedFromAddress)))
                    {
        ...
        public void SendSIPReliable(SIPTransaction sipTransaction)
        {
            ...
            {
                throw new ApplicationException(
                    "Cannot send reliable SIP message as the reliable transmissions queue is full.");
            }
                ...
                sipTransaction.TransactionState == SIPTransactionStatesEnum.Completed)
            {
                ...
                {
                    throw new ApplicationException(
                        "The SIPTransport layer cannot send a reliable SIP response because the send from socket has not been set for the transaction.");
                }
                else
                {
        ...
        public void SendResponse(SIPEndPoint dstEndPoint, SIPResponse sipResponse)
        {
            ...
            else
            {
                ...
                {
                    throw new ApplicationException(
                        "No channels are configured in the SIP transport layer. The response could not be sent.");
                }
        ...
        public void SendResponse(SIPResponse sipResponse)
        {
            ...
            else
            {
                ...
                {
                    throw new ApplicationException("Could not find a SIP channel to send SIP Response to " +
                                                   topViaHeader.ReceivedFromAddress + ".");
                }
            }
        }
        ...
        private void SendResponse(SIPChannel sipChannel, SIPResponse sipResponse)
        {
            ...
            else
            {
                ...
                {
                    SendResponse(sipChannel, new SIPEndPoint(topVia.Transport, dstEndPoint.GetIPEndPoint()),
                        sipResponse);
                }
                else
                {
            ...
            try
            {
                while (!m_closed)
                {
                    ...
                    while (m_inMessageQueue.Count > 0)
                    {
                        ...
                        {
                            SIPMessageReceived(incomingMessage.LocalSIPChannel, incomingMessage.RemoteEndPoint,
                        ...
                        lock (m_reliableTransmissions)
                        {
                            foreach (SIPTransaction transaction in m_reliableTransmissions.Values)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        double nextTransmitMilliseconds =
                                            Math.Pow(2, transaction.Retransmits - 1) * m_t1;
                                        nextTransmitMilliseconds = (nextTransmitMilliseconds > m_t2)
                                            ? m_t2
                                            : nextTransmitMilliseconds;
                                        ...
                                        if (DateTime.Now.Subtract(transaction.LastTransmit).TotalMilliseconds >=
                                            nextTransmitMilliseconds)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    SendRequest(transaction.OutboundProxy,
                                                        transaction.TransactionRequest);
                                                }
                                                else
                                                {
                                                    SendRequest(transaction.RemoteEndPoint,
                                                        transaction.TransactionRequest);
                                                }
            ...
            try
            {
                if (buffer != null && buffer.Length > 0)
                {
                    if ((buffer[0] == 0x0 || buffer[0] == 0x1) && buffer.Length >= 20)
                    {
                        ...
                        {
                            STUNRequestReceived(sipChannel.SIPChannelEndPoint.GetIPEndPoint(),
                                remoteEndPoint.GetIPEndPoint(), buffer, buffer.Length);

                            if (PerformanceMonitorPrefix != null)
                            {
                                SIPSorceryPerformanceMonitor.IncrementCounter(
                                    PerformanceMonitorPrefix + SIPSorceryPerformanceMonitor
                                        .SIP_TRANSPORT_STUN_REQUESTS_PER_SECOND_SUFFIX);
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (buffer.Length > SIPConstants.SIP_MAXIMUM_RECEIVE_LENGTH)
                        {
                            ...
                            FireSIPBadRequestInTraceEvent(sipChannel.SIPChannelEndPoint, remoteEndPoint,
                                ...
                                rawErrorMessage);
                            SIPResponse tooLargeResponse = GetResponse(sipChannel.SIPChannelEndPoint, remoteEndPoint,
                            ...
                            if (PerformanceMonitorPrefix != null)
                            {
                                SIPSorceryPerformanceMonitor.IncrementCounter(
                                    PerformanceMonitorPrefix + SIPSorceryPerformanceMonitor
                                        .SIP_TRANSPORT_SIP_BAD_MESSAGES_PER_SECOND_SUFFIX);
                            }
                        }
                        else
                        {
                            ...
                            Encoding.RegisterProvider(CodePagesEncodingProvider
                                .Instance); //函数进行注册,解使用GB2312编码异常，配合System.Text.Encoding.CodePages包使用
                            rawSIPMessage = Encoding.GetEncoding("GB2312").GetString(buffer, 0, buffer.Length);
                            if (!rawSIPMessage.StartsWith("REGISTER", StringComparison.CurrentCultureIgnoreCase) &&
                                !rawSIPMessage.StartsWith("MESSAGE", StringComparison.CurrentCultureIgnoreCase))
                            {
                            ...
                            {
                                FireSIPBadRequestInTraceEvent(sipChannel.SIPChannelEndPoint, remoteEndPoint,
                                    "Missing SIP string.", SIPValidationFieldsEnum.NoSIPString, rawSIPMessage);
                            ...
                            SIPMessage sipMessage = SIPMessage.ParseSIPMessage(rawSIPMessage,
                                sipChannel.SIPChannelEndPoint, remoteEndPoint);
                            ...
                            if (sipMessage != null)
                            {
                                if (sipMessage.SIPMessageType == SIPMessageTypesEnum.Response)
                                {
                                    ...
                                    try
                                    {
                                        if (PerformanceMonitorPrefix != null)
                                        {
                                            SIPSorceryPerformanceMonitor.IncrementCounter(
                                                PerformanceMonitorPrefix + SIPSorceryPerformanceMonitor
                                                    .SIP_TRANSPORT_SIP_RESPONSES_PER_SECOND_SUFFIX);
                                        }
                                        ...
                                        {
                                            FireSIPResponseInTraceEvent(sipChannel.SIPChannelEndPoint, remoteEndPoint,
                                                sipResponse);
                                        }
                                        ...
                                        if (_transactionEngine != null && _transactionEngine.Exists(sipResponse))
                                        {
                                            ...
                                            transaction.GotResponse(sipChannel.SIPChannelEndPoint, remoteEndPoint,
                                                sipResponse);
                                        }
                                        else
                                        {
                                    ...
                                    catch (SIPValidationException sipValidationException)
                                    {
                                        ...
                                        FireSIPBadResponseInTraceEvent(sipChannel.SIPChannelEndPoint, remoteEndPoint,
                                            sipMessage.RawMessage, sipValidationException.SIPErrorField,
                                            sipMessage.RawMessage);
                                    }
                                ...
                                else
                                {
                                    ...
                                    if (PerformanceMonitorPrefix != null)
                                    {
                                        SIPSorceryPerformanceMonitor.IncrementCounter(
                                            PerformanceMonitorPrefix + SIPSorceryPerformanceMonitor
                                                .SIP_TRANSPORT_SIP_REQUESTS_PER_SECOND_SUFFIX);
                                    }
                                    ...
                                    try
                                    {
                                        ...
                                        if (!sipRequest.IsValid(out sipRequestErrorField,
                                            out string sipRequestValidationError))
                                        {
                                        ...
                                        {
                                            FireSIPRequestInTraceEvent(sipChannel.SIPChannelEndPoint, remoteEndPoint,
                                                sipRequest);
                                        }
                                        ...
                                        SIPTransaction requestTransaction =
                                            _transactionEngine?.GetTransaction(sipRequest);
                                        if (requestTransaction != null)
                                        {
                                            ...
                                            else if (sipRequest.Method == SIPMethodsEnum.ACK)
                                            {
                                                    ...
                                                    SIPTransactionStatesEnum.Completed)
                                                {
                                                    ...
                                                    requestTransaction.ACKReceived(sipChannel.SIPChannelEndPoint,
                                                        remoteEndPoint, sipRequest);
                                                }
                                                else if (requestTransaction.TransactionState ==
                                                         SIPTransactionStatesEnum.Confirmed)
                                                {
                                                ...
                                                else
                                                {
                                                    ...
                                                    FireSIPBadRequestInTraceEvent(sipChannel.SIPChannelEndPoint,
                                                        remoteEndPoint,
                                                        "ACK recieved on " + requestTransaction.TransactionState +
                                        ...
                                        else if (SIPTransportRequestReceived != null)
                                        {
                                                ...
                                                sipRequest.Method != SIPMethodsEnum.OPTIONS)
                                            {
                                                ...
                                                FireSIPBadRequestInTraceEvent(sipChannel.SIPChannelEndPoint,
                                                    remoteEndPoint,
                                                    "Zero MaxForwards on " + sipRequest.Method + " " +
                                                    sipRequest.URI.ToString() + " from " +
                                                    sipRequest.Header.From.FromURI.User + " " +
                                                    remoteEndPoint.ToString(), SIPValidationFieldsEnum.Request,
                                                    sipRequest.ToString());
                                                SIPResponse tooManyHops = GetResponse(sipRequest,
                                            ...
                                            SIPTransportRequestReceived?.Invoke(sipChannel.SIPChannelEndPoint,
                                                remoteEndPoint, sipRequest);
                                        }
                                    }
                                    catch (SIPValidationException sipRequestExcp)
                                    {
                                        FireSIPBadRequestInTraceEvent(sipChannel.SIPChannelEndPoint, remoteEndPoint,
                                            ...
                                            sipMessage.RawMessage);
                                        SIPResponse errorResponse = GetResponse(sipChannel.SIPChannelEndPoint,
        ...
        private SIPChannel GetDefaultChannel(SIPProtocolsEnum protocol)
        {
            ...
            {
                if (sipChannel.SIPChannelEndPoint.Protocol == protocol &&
                    !IPAddress.IsLoopback(sipChannel.SIPChannelEndPoint.Address))
                {
                    return sipChannel;
                }
            }
        ...
        private void FireSIPRequestInTraceEvent(SIPEndPoint localSIPEndPoint, SIPEndPoint remoteEndPoint,
        ...
        private void FireSIPRequestOutTraceEvent(SIPEndPoint localSIPEndPoint, SIPEndPoint remoteEndPoint,
            SIPRequest sipRequest)
        {
            try
            {
        ...
        private void FireSIPResponseInTraceEvent(SIPEndPoint localSIPEndPoint, SIPEndPoint remoteEndPoint,
        ...
        private void FireSIPResponseOutTraceEvent(SIPEndPoint localSIPEndPoint, SIPEndPoint remoteEndPoint,
            SIPResponse sipResponse)
        {
            try
            {
        ...
        private void FireSIPBadRequestInTraceEvent(SIPEndPoint localSIPEndPoint, SIPEndPoint remoteEndPoint,
            string message, SIPValidationFieldsEnum sipErrorField, string rawMessage)
        {
            try
            {
        ...
        private void FireSIPBadResponseInTraceEvent(SIPEndPoint localSIPEndPoint, SIPEndPoint remoteEndPoint,
            string message, SIPValidationFieldsEnum sipErrorField, string rawMessage)
        {
            try
            {
                SIPBadResponseInTraceEvent?.Invoke(localSIPEndPoint, remoteEndPoint, message, sipErrorField,
                    rawMessage);
            }
            catch (Exception excp)
            {
        ...
        /// </summary>
        public SIPResponse GetResponse(SIPEndPoint localSIPEndPoint, SIPEndPoint remoteEndPoint,
            SIPResponseStatusCodesEnum responseCode, string reasonPhrase)
        {
            try
            {
                ...
                response.Header.Vias.PushViaHeader(new SIPViaHeader(
                    new SIPEndPoint(localSIPEndPoint.Protocol, remoteEndPoint.GetIPEndPoint()),
                    CallProperties.CreateBranchId()));
                response.Header.MaxForwards = Int32.MinValue;
                response.Header.Allow = ALLOWED_SIP_METHODS;
        ...
        public SIPRequest GetRequest(SIPMethodsEnum method, SIPURI uri, SIPToHeader to, SIPEndPoint localSIPEndPoint)
        {
            try
            {
                ...
                SIPContactHeader contactHeader =
                    new SIPContactHeader(null, new SIPURI(SIPSchemesEnum.sip, localSIPEndPoint));
                SIPFromHeader fromHeader =
                    new SIPFromHeader(null, contactHeader.ContactURI, CallProperties.CreateNewTag());
        ...
        public SIPNonInviteTransaction CreateNonInviteTransaction(SIPRequest sipRequest, SIPEndPoint dstEndPoint,
            SIPEndPoint localSIPEndPoint, SIPEndPoint outboundProxy)
        {
            try
            {
                if (localSIPEndPoint == null)
                ...
                CheckTransactionEngineExists();
                SIPNonInviteTransaction nonInviteTransaction =
                    new SIPNonInviteTransaction(this, sipRequest, dstEndPoint, localSIPEndPoint, outboundProxy);
        ...
        public UACInviteTransaction CreateUACTransaction(SIPRequest sipRequest, SIPEndPoint dstEndPoint,
            SIPEndPoint localSIPEndPoint, SIPEndPoint outboundProxy, bool sendOkAckManually = false)
        {
            try
            {
                if (localSIPEndPoint == null)
                ...
                CheckTransactionEngineExists();
                UACInviteTransaction uacInviteTransaction = new UACInviteTransaction(this, sipRequest, dstEndPoint,
                    localSIPEndPoint, outboundProxy, sendOkAckManually);
        ...
        public UASInviteTransaction CreateUASTransaction(SIPRequest sipRequest, SIPEndPoint dstEndPoint,
            SIPEndPoint localSIPEndPoint, SIPEndPoint outboundProxy, bool noCDR = false)
        {
            try
            {
                if (localSIPEndPoint == null)
                ...
                CheckTransactionEngineExists();
                UASInviteTransaction uasInviteTransaction = new UASInviteTransaction(this, sipRequest, dstEndPoint,
                    localSIPEndPoint, outboundProxy, ContactIPAddress, noCDR);
                if (_transactionEngine.GetTransaction(sipRequest) != null
                    && _transactionEngine.GetTransaction(sipRequest).TransactionId
                        .Equals(uasInviteTransaction.TransactionId))
                {
        ...
        public SIPCancelTransaction CreateCancelTransaction(SIPRequest sipRequest, SIPEndPoint dstEndPoint,
            SIPEndPoint localSIPEndPoint, UASInviteTransaction inviteTransaction)
        {
            try
            {
                if (localSIPEndPoint == null)
                ...
                CheckTransactionEngineExists();
                SIPCancelTransaction cancelTransaction = new SIPCancelTransaction(this, sipRequest, dstEndPoint,
                    localSIPEndPoint, inviteTransaction);
        ...
        private void CheckTransactionEngineExists()
        {
            ...
            {
                throw new ApplicationException(
                    "A transaction engine is required for this operation but one has not been provided.");
            }
        }
