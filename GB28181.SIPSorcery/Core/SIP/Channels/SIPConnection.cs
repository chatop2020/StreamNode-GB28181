...
namespace GB28181
{
    ...
    public class SIPConnection
    {
        ...
        public SIPConnection(SIPChannel channel, TcpClient tcpClient, Stream sipStream, IPEndPoint remoteEndPoint,
            SIPProtocolsEnum connectionProtocol, SIPConnectionsEnum connectionType)
        {
            LastTransmission = DateTime.Now;
            m_owningChannel = channel;
        ...
        public static int GetContentLength(byte[] buffer, int start, int end)
        {
            ...
            else
            {
                byte[] contentHeaderBytes =
                    Encoding.UTF8.GetBytes(m_sipEOL + SIPHeaders.SIP_HEADER_CONTENTLENGTH.ToUpper());
                byte[] compactContentHeaderBytes =
                    Encoding.UTF8.GetBytes(m_sipEOL + SIPHeaders.SIP_COMPACTHEADER_CONTENTLENGTH.ToUpper());
