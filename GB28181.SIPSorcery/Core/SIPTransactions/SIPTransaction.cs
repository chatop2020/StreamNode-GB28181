...
namespace GB28181
{
    public enum SIPTransactionStatesEnum
    {
        ...
        Cancelled =
            7, // This state is not in the SIP RFC but is deemed the most practical way to record that an INVITE has been cancelled. Other states will have ramifications for the transaction lifetime.
    }
    ...
    public class SIPTransaction
    {
       // protected static ILog logger = AssemblyState.logger;
        ...
        public bool
            DeliveryFailed =
                false; // If the transport layer does not receive a response to the request in the alloted time the request will be marked as failed.
        ...
        private string
            m_sentBy; // The contact address from the top Via header that created the transaction. This is used for matching requests to server transactions.
        ...
        public DateTime
            CompletedAt =
                DateTime.Now; // For INVITEs thiis the time they recieved the final response and is used to calculate the time they expie as T6 after this.
        ...
        public SIPEndPoint
            RemoteEndPoint; // The remote socket that caused the transaction to be created or the socket a newly created transaction request was sent to.             

        public SIPEndPoint
            LocalSIPEndPoint; // The local SIP endpoint the remote request was received on the if created by this stack the local SIP end point used to send the transaction.
        ...
        protected SIPResponse
            m_transactionFinalResponse; // This is the final response being sent by a UAS transaction or the one received by a UAC one.
            ...
            SIPEndPoint outboundProxy)
        {
            try
            {
                ...
                {
                    throw new ArgumentNullException(
                        "The local SIP end point must be set when creating a SIPTransaction.");
                }
        ...
        private void UpdateTransactionState(SIPTransactionStatesEnum transactionState)
        {
            ...
            if (TransactionStateChanged != null && transactionState != SIPTransactionStatesEnum.Proceeding &&
        ...
        public virtual void SendFinalResponse(SIPResponse finalResponse)
        {
            ...
            {
                FireTransactionTraceMessage("Send Final Response Reliable " + LocalSIPEndPoint.ToString() + "->" +
                                            viaAddress + m_crLF + finalResponse.ToString());
                m_sipTransport.SendSIPReliable(this);
            }
            else
            {
                FireTransactionTraceMessage("Send Final Response " + LocalSIPEndPoint.ToString() + "->" + viaAddress +
                                            m_crLF + finalResponse.ToString());
        ...
        {
            FireTransactionTraceMessage("Send Info Response " + LocalSIPEndPoint.ToString() + "->" +
                                        this.RemoteEndPoint + m_crLF + sipResponse.ToString());
        ...
        {
            FireTransactionTraceMessage("Send Request " + LocalSIPEndPoint.ToString() + "->" + dstEndPoint + m_crLF +
                                        sipRequest.ToString());
        ...
        public void RequestRetransmit()
        {
            ...
            FireTransactionTraceMessage("Send Request retransmit " + Retransmits + " " + LocalSIPEndPoint.ToString() +
                                        "->" + this.RemoteEndPoint + m_crLF + this.TransactionRequest.ToString());
        }
        ...
        private void ResponseRetransmit()
        {
            ...
            FireTransactionTraceMessage("Send Response retransmit " + LocalSIPEndPoint.ToString() + "->" +
                                        this.RemoteEndPoint + m_crLF + this.TransactionFinalResponse.ToString());
        }
 ...
 sipTransport.CreateUACTransaction(request, dummySIPEndPoint, dummySIPEndPoint, null);
