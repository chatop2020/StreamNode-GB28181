...
namespace GB28181.App
{
    public class SIPPresenceEventSubscription : SIPEventSubscription
    {
        ...
        public const string
            SWITCHBOARD_FILTER =
                "switchboard"; // If a client specifies this value as a filter it's only interested in SIP accounts that are switchboard enabled.
        ...
        private bool
            m_switchboardSIPAccountsOnly; // If true means this subscription should only generate notifications for SIP accounts that are switchboard enabled.
            ...
            try
            {
                ...
                if (ResourceURI.User == m_wildcardUser)
                {
                    if (m_switchboardSIPAccountsOnly)
                    {
                        ...
                        sipAccounts = GetSIPAccounts_External(
                            s => s.Owner == SubscriptionDialogue.Owner && s.IsSwitchboardEnabled, "SIPUsername", 0,
                            MAX_SIPACCOUNTS_TO_RETRIEVE);
                    }
                    else
                    {
                        //sipAccounts = m_sipAccountPersistor.Get(s => s.Owner == SubscriptionDialogue.Owner, "SIPUsername", 0, MAX_SIPACCOUNTS_TO_RETRIEVE);
                        sipAccounts = GetSIPAccounts_External(s => s.Owner == SubscriptionDialogue.Owner, "SIPUsername",
                            0, MAX_SIPACCOUNTS_TO_RETRIEVE);
                    }
                }
                else
                {
                    if (m_switchboardSIPAccountsOnly)
                    {
                        ...
                        sipAccounts = GetSIPAccounts_External(
                            s => s.SIPUsername == CanonicalResourceURI.User &&
                                 s.SIPDomain == CanonicalResourceURI.Host && s.IsSwitchboardEnabled, "SIPUsername", 0,
                            MAX_SIPACCOUNTS_TO_RETRIEVE);
                    }
                    else
                    {
                        ...
                        sipAccounts = GetSIPAccounts_External(
                            s => s.SIPUsername == CanonicalResourceURI.User && s.SIPDomain == CanonicalResourceURI.Host,
                            "SIPUsername", 0, MAX_SIPACCOUNTS_TO_RETRIEVE);
                    }
                }
                ...
                foreach (SIPAccount sipAccount in sipAccounts)
                {
                    ...
                    if (bindingsCount > 0)
                    {
                        ...
                        Presence.Tuples.Add(new SIPEventPresenceTuple(safeSIPAccountID, SIPEventPresenceStateEnum.open,
                            aor, Decimal.Zero, sipAccount.AvatarURL));
                        //logger.Debug(" full presence " + aor.ToString() + " open.");
                    }
                    else
                    {
                        string safeSIPAccountID = sipAccount.Id.ToString();
                        Presence.Tuples.Add(new SIPEventPresenceTuple(safeSIPAccountID,
                            SIPEventPresenceStateEnum.closed, null, Decimal.Zero, sipAccount.AvatarURL));
            ...
            try
            {
                ...
                if (m_switchboardSIPAccountsOnly)
                {
                    ...
                    sendNotificationForEvent =
                        Convert.ToBoolean(GetSipAccountProperty_External(sipAccountID, "IsSwitchboardEnabled"));
                ...
                if (sendNotificationForEvent)
                {
                    if (machineEvent.MachineEventType == SIPMonitorMachineEventTypesEnum.SIPRegistrarBindingUpdate)
                    {
                        ...
                        Presence.Tuples.Add(new SIPEventPresenceTuple(safeSIPAccountID, SIPEventPresenceStateEnum.open,
                            sipAccountURI, Decimal.Zero, avatarURL));
                    ...
                    else
                    {
                        ...
                        {
                            Presence.Tuples.Add(new SIPEventPresenceTuple(safeSIPAccountID,
                                SIPEventPresenceStateEnum.open, sipAccountURI, Decimal.Zero, avatarURL));
                        }
                        else
                        {
                            Presence.Tuples.Add(new SIPEventPresenceTuple(safeSIPAccountID,
                                SIPEventPresenceStateEnum.closed, sipAccountURI, Decimal.Zero, avatarURL));
                        }
                    }
