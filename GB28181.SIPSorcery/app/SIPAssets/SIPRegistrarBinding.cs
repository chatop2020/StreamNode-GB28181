    ...
    public class SIPRegistrarBinding : INotifyPropertyChanged, ISIPAsset
    {
        ...
        public const int
            MAX_BINDING_LIFETIME =
                3600; // Bindings are currently not being expired once the expires time is reached and this is the maximum amount of time 
        ...
        private static Dictionary<string, int>
            m_userAgentExpirys =
                new Dictionary<string, int>(); // Result of parsing user agent expiry values from the App.Config Xml Node.
            ...
            SIPAccountName
        {
            get;
            set;
        } // Used for informational purposes only, no matching done against it and should not be relied on. Use SIPAccountId instead.
        ...
        [IgnoreDataMember] public string CallId;

        [IgnoreDataMember] public int CSeq;
        ...
        {
            get { return m_lastUpdate.AddSeconds(m_expiry); }
            set
            {
            } // The expiry time is stored in the database for info. It is calculated from expiry and lastudpate and does not need a setter.
        }

        [DataMember]
        public string
            Q // The Q value on the on the Contact header to indicate relative priority among bindings for the same address of record.
        {
            get
            {
        ...
        [IgnoreDataMember]
        public SIPEndPoint
            ProxySIPEndPoint // This is the socket the request was received from and assumes that the prior SIP element was a SIP proxy.
        {
        ...
        public void Load(DataRow row)
        {
            try
            {
                ...
                m_mangledContactURI = (!(row["mangledcontacturi"] as string).IsNullOrBlank())
                    ? SIPURI.ParseSIPURI(row["mangledcontacturi"] as string)
                    : null;
                Expiry = Convert.ToInt32(row["expiry"]);
                RemoteSIPEndPoint = (!(row["remotesipsocket"] as string).IsNullOrBlank())
                    ? SIPEndPoint.ParseSIPEndPoint(row["remotesipsocket"] as string)
                    : null;
                m_proxySIPEndPoint = (!(row["proxysipsocket"] as string).IsNullOrBlank())
                    ? SIPEndPoint.ParseSIPEndPoint(row["proxysipsocket"] as string)
                    : null;
                m_registrarSIPEndPoint = (!(row["registrarsipsocket"] as string).IsNullOrBlank())
                    ? SIPEndPoint.ParseSIPEndPoint(row["registrarsipsocket"] as string)
                    : null;
        ...
        /// </summary>
        public void RefreshBinding(int expiry, SIPEndPoint remoteSIPEndPoint, SIPEndPoint proxySIPEndPoint,
            SIPEndPoint registrarSIPEndPoint, bool dontMangle)
        {
        ...
        public string ToContactString()
        {
            ...
            return "<" + m_contactURI.ToString() + ">;" + SIPContactHeader.EXPIRES_PARAMETER_KEY + "=" +
                   secondsRemaining;
        }
