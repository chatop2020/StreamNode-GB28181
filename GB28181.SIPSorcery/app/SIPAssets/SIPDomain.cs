...
namespace GB28181.App
{
    ...
    public class SIPDomain : ISIPAsset
    {
        ...
        public string AliasList
        {
            ...
            set { Aliases = ParseAliases(value); }
        }
        ...
        public void Load(DataRow row)
        {
            Id = (row.Table.Columns.Contains("id") && row["id"] != DBNull.Value)
                ? new Guid(row["id"] as string)
                : Guid.NewGuid();
            Domain = row["domain"] as string;
            Owner = (row.Table.Columns.Contains("owner") && row["owner"] != DBNull.Value)
                ? row["owner"] as string
                : null;
            if (row.Table.Columns.Contains("inserted") & row["inserted"] != DBNull.Value &&
                !(row["inserted"] as string).IsNullOrBlank())
            {
                Inserted = DateTimeOffset.Parse(row["inserted"] as string);
            }

            string aliasList = (row.Table.Columns.Contains("aliaslist") & row["aliaslist"] != DBNull.Value)
                ? row["aliaslist"] as string
                : null;
