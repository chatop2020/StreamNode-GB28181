...
namespace GB28181.App
{
    ...
    public class SIPAccount : INotifyPropertyChanged, ISIPAsset
    {
        ...
        public ushort MediaPortMin
        {
            get { return MediaPortMin; }
            set
            {
                MediaPortMin = value;
                SIPMessageCore.MEDIA_PORT_START = value;
            }
        }

        public ushort MediaPortMax
        {
            get { return MediaPortMax; }
            set
            {
                MediaPortMax = value;
                SIPMessageCore.MEDIA_PORT_END = value;
            }
        }
        ...
        private string
            m_inDialPlanName; // The dialplan that will be used for incoming calls. If this field is empty incoming calls will be forwarded to the account's current bindings.
        ...
        private string
            m_networkId; // SIP accounts with the ame network id will not have their Contact headers or SDP mangled for private IP address.
        ...
        private string
            m_ipAddressACL; // A regular expression that acts as an IP address Access Control List for SIP request authorisation.
            ...
            string outDialPlanName)
        {
            try
            {
                ...
                m_outDialPlanName = (outDialPlanName != null && outDialPlanName.Trim().Length > 0)
        ...
        public static string ValidateAndClean(SIPAccount sipAccount)
        {
            ...
            else if (Regex.Match(sipAccount.SIPUsername, "[^" + USERNAME_ALLOWED_CHARS + "]").Success)
            {
                return "The username had an invalid character, characters permitted are alpha-numeric and .-_.";
                     ...
                     (sipAccount.SIPUsername.Substring(sipAccount.SIPUsername.LastIndexOf(".") + 1).Trim().Length >=
                      USERNAME_MIN_LENGTH &&
                      sipAccount.SIPUsername.Substring(sipAccount.SIPUsername.LastIndexOf(".") + 1).Trim() !=
                      sipAccount.Owner))
            {
                return "You are not permitted to create this username. Only user " +
                       sipAccount.SIPUsername.Substring(sipAccount.SIPUsername.LastIndexOf(".") + 1).Trim() +
                       " can create SIP accounts ending in " +
                       sipAccount.SIPUsername.Substring(sipAccount.SIPUsername.LastIndexOf(".")).Trim() + ".";
            }
            else if (!sipAccount.IsIncomingOnly || !sipAccount.SIPPassword.IsNullOrBlank())
            {
                ...
                else if (sipAccount.SIPPassword.Length < PASSWORD_MIN_LENGTH ||
                         sipAccount.SIPPassword.Length > PASSWORD_MAX_LENGTH)
                {
                    return "The password field must be at least " + PASSWORD_MIN_LENGTH +
                           " characters and no more than " + PASSWORD_MAX_LENGTH + " characters.";
                }
                else
                {
                    ...
                    if (illegalCharFound)
                    {
                        return "Your password has an invalid character " + illegalChar +
                               " it can only contain a to Z, 0 to 9 and characters in this set " +
                               SafeXML.MakeSafeXML(new String(NONAPLPHANUM_ALLOWED_PASSWORD_CHARS)) + ".";
                    }
                }
            }
