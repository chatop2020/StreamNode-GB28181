...
namespace GB28181.Net
{
    public class RTSPServer
    {
        ...
        private const int
            INITIALPRUNE_CONNECTIONS_DELAY =
                60000; // Wait this long before starting the prune checks, there will be no connections to prune initially and the CPU is needed elsewhere.
        ...
        private const int
            CONNECTION_ATTEMPTS_ALLOWED =
                3; // The number of failed connection attempts permitted before classifying a remote socket as failed.

        private const int
            FAILED_CONNECTION_DONTUSE_INTERVAL =
                300; // If a socket cannot be connected to don't try and reconnect to it for this interval.

        private const int
            CLOSE_RTSP_SESSION_NO_DATA_SECONDS =
                60; // The number of seconds after which to close an RTSP session if nothing is received on the RTP or control sockets.
        ...
        private Dictionary<string, RTSPConnection>
            m_connectedSockets =
                new Dictionary<string, RTSPConnection>(); // Keeps a track of the TCP current TCP connections that have been establised by RTSP clients.

        private Dictionary<string, RTSPSession>
            m_rtspSessions =
                new Dictionary<string, RTSPSession>(); // Keeps a track of the current RTSP sessions that have been established by this server.
        ...
        private void ReceiveCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                {
                    rtspConnection.Stream.BeginRead(rtspConnection.SocketBuffer, rtspConnection.SocketBufferEndPosition,
                        MaxMessageSize - rtspConnection.SocketBufferEndPosition, new AsyncCallback(ReceiveCallback),
                        rtspConnection);
                }
            }
