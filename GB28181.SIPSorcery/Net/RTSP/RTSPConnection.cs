...
namespace GB28181.Net
{
    public class RTSPConnection
    {
                ...
                if (bytesRead > 0)
                {
                    ...
                    while (rtspMsgBuffer != null)
                    {
                        ...
                        else
                        {
                            ...
                            Array.Copy(SocketBuffer, rtspMsgBuffer.Length + bytesSkipped, SocketBuffer, 0,
        ...
        public static byte[] ProcessReceive(byte[] receiveBuffer, int start, int length, out int bytesSkipped)
        {
            ...
            {
                int endMessageIndex =
                    ByteBufferInfo.GetStringPosition(receiveBuffer, start, length, m_rtspMessageDelimiter, null);
        ...
        public static int GetContentLength(byte[] buffer, int start, int end)
        {
            ...
            else
            {
                byte[] contentHeaderBytes =
                    Encoding.UTF8.GetBytes(m_rtspEOL + RTSPHeaders.RTSP_HEADER_CONTENTLENGTH.ToUpper());
