...
namespace GB28181.Net
{
    public class RTSPSession
    {
        ...
        private const int
            MEDIA_PORT_START = 30000; // Arbitrary port number to start allocating RTP and control ports from.

        private const int
            MEDIA_PORT_END = 40000; // Arbitrary port number that RTP and control ports won't be allocated above.
        ...
        private const int
            MAXIMUM_RTP_PORT_BIND_ATTEMPTS =
                3; // The maximum number of re-attempts that will be made when trying to bind the RTP port.
        ...
        private int
            _rtpPayloadHeaderLength =
                0; // Some RTP media types use a payload header to carry information about the encoded media. Typically this header needs to be stripped off before passing to a decoder.
            ...
            DontTimeout
        {
            get;
            set;
        } // If set to true means a server should not timeout this session even if no activity is received on the RTP socket.
        ...
        public Action<byte[], int, Action<byte[]>>
            OnDtlsReceive; // [buffer, length, Raw send callback method] Handler for any DTLS packets received.                

        public Func<byte[], byte[]>
            RtpProtect; // [out encrypted buffer, in buffer] Set this for SRTP protection of RTP packets.
            ...
            lock (_allocatePortsMutex)
            {
                ...
                if (_rtpPort != 0 && _controlPort != 0)
                {
                    ...
                    for (int bindAttempts = 0; bindAttempts <= MAXIMUM_RTP_PORT_BIND_ATTEMPTS; bindAttempts++)
                    {
                        ...
                        {
                            Logger.Logger.Error("RTSP session " + _sessionID + " failed to bind to RTP port " +
                                                _rtpPort +
                                                " and/or control port of " + _controlPort + ", attempt " +
                                                bindAttempts + ". " +
                                                sockExcp);
                    ...
                    if (!bindSuccess)
                    {
                        throw new ApplicationException(
                            "An RTSP session could not bind to the RTP and/or control ports within the range of " +
                            MEDIA_PORT_START + " to " + MEDIA_PORT_END + ".");
                    }
                }
                else
                {
                    throw new ApplicationException(
                        "An RTSP session could not allocate the RTP and/or control ports within the range of " +
                        MEDIA_PORT_START + " to " + MEDIA_PORT_END + ".");
                }
            }
        }
        ...
        public void Start()
        {
            if (_rtpSocket != null && _controlSocket != null)
            {
                ...
                _controlSocket.BeginReceive(_controlSocketBuffer, 0, _controlSocketBuffer.Length, SocketFlags.None,
                    out _controlSocketError, ControlSocketReceive, null);
            }
            else
            {
                Logger.Logger.Warn(
                    "An RTSP session could not start as either RTP or control sockets were not available.");
            }
        }
            ...
            try
            {
                ...
                while (!_closed)
                {
                    try
                    {
                        ...
                        {
                            Logger.Logger.Warn("Zero bytes read from RTSPSession RTP socket for session ID " +
                                               SessionID +
        ...
        public void SendH264Frame(byte[] frame, int payloadType)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (_rtcpRemoteEndPoint != null && DateTime.Now.Subtract(_senderLastSentAt).TotalSeconds >
                        RTCP_SENDER_REPORT_INTERVAL_SECONDS)
                    {
                    ...
                    Console.WriteLine("Sending " + frame.Length + " H264 encoded bytes to client, timestamp " +
                                      _timestamp + ", starting sequence number " + _sequenceNumber + ".");
                    ...
                    for (int index = 0; index * RTP_MAX_PAYLOAD < frame.Length; index++)
                    {
                        ...
                        _rtpSocket.BeginSendTo(rtpBytes, 0, rtpBytes.Length, SocketFlags.None, _remoteEndPoint, null,
                            null);
                ...
                else
                {
                    ...
                    for (int index = 0; index * RTP_MAX_PAYLOAD < frame.Length; index++)
                    {
                        byte[] vp8HeaderBytes = (index == 0)
                            ? new byte[VP8_RTP_HEADER_LENGTH] {0x90, 0x80, (byte) (_sequenceNumber % 128)}
                            : new byte[VP8_RTP_HEADER_LENGTH] {0x80, 0x80, (byte) (_sequenceNumber % 128)};
        ...
        public void SendRTPRaw(byte[] payload)
        {
            try
            {
                if (!_closed && _rtpSocket != null && _remoteEndPoint != null && _rtpSocketError == SocketError.Success)
                {
                    ...
                    _rtpSocket.BeginSendTo(payload, 0, payload.Length, SocketFlags.None, _remoteEndPoint,
                        SendRtpCallback, _rtpSocket);
                }
            }
            catch (Exception excp)
            {
        ...
        private void SendRtcpSenderReport(ulong ntpTimestamp, uint rtpTimestamp)
        {
            try
            {
                Console.WriteLine("Sending RTCP sender report to remote, ntp timestamp " + ntpTimestamp +
                                  ", rtp timestamp " + rtpTimestamp + ", packet count " + _senderPacketCount + ".");

                RTCPPacket senderReport = new RTCPPacket(_syncSource, ntpTimestamp, rtpTimestamp, _senderPacketCount,
                    _senderOctetCount);
                var bytes = senderReport.GetBytes();

                _controlSocket.BeginSendTo(bytes, 0, bytes.Length, SocketFlags.None, _rtcpRemoteEndPoint,
                    SendRtcpCallback, _controlSocket);
        ...
        private void SendRtpCallback(IAsyncResult ar)
        {
            ...
            catch (Exception excp)
            {
                if (!_closed)
                {
                    Logger.Logger.Error("Exception RTSPSession.SendRtpCallback attempting to send to " +
                                        _remoteEndPoint +
                                        ". ->" + excp);
        ...
        private void SendRtcpCallback(IAsyncResult ar)
        {
            ...
            catch (Exception excp)
            {
                if (!_closed)
                {
                    Logger.Logger.Error("Exception RTSPSession.SendRtcpCallback attempting to send to " +
                                        _rtcpRemoteEndPoint +
                                        ". ->" + excp);
