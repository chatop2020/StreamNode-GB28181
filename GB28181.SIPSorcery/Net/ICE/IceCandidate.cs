...
namespace GB28181.Net
{
    ...
    public class IceCandidate
    {
        ...
        public bool
            IsStunLocalExchangeComplete; // This is the authenticated STUN request sent by us to the remote WebRTC peer.

        public bool
            IsStunRemoteExchangeComplete; // This is the authenticated STUN request sent by the remote WebRTC peer to us.
        ...
        public override string ToString()
        {
            ...
            var candidateStr =
                $"a=candidate:{Crypto.GetRandomInt(10)} 1 udp {Crypto.GetRandomInt(10)} {LocalAddress} {(LocalRtpSocket.LocalEndPoint as IPEndPoint).Port} typ host generation 0\r\n";

            if (StunRflxIPEndPoint != null)
            {
                candidateStr += string.Format(
                    "a=candidate:{0} {1} udp {2} {3} {4} typ srflx raddr {5} rport {6} generation 0\r\n",
                    Crypto.GetRandomInt(10).ToString(), "1", Crypto.GetRandomInt(10).ToString(),
                    StunRflxIPEndPoint.Address, StunRflxIPEndPoint.Port, LocalAddress.ToString(),
                    (LocalRtpSocket.LocalEndPoint as IPEndPoint).Port);
