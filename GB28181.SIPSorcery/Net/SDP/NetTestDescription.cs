...
namespace GB28181.Net
{
    public class NetTestDescription
    {
        ...
        public NetTestDescription(Guid testId, string clientSocket, string clientISP, string serverSocket,
            string serverISP, string username, string comment)
        {
            TestId = testId;
        ...
        {
            string netTestString = TestId.ToString() + DELIMITER_CHARACTER + ClientSocket + DELIMITER_CHARACTER +
                                   ServerSocket + DELIMITER_CHARACTER +
                                   ClientISP + DELIMITER_CHARACTER + ServerISP + DELIMITER_CHARACTER + Username +
                                   DELIMITER_CHARACTER + Comment;
