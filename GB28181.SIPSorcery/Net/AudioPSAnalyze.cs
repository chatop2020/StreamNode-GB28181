        ...
        public int Gb28181_streampackageForH264(byte[] pData, int nFrameLen, Data_Info_s pPacker, int stream_type)
        {
            try
            {
                ...
                while (nFrameLen > 0)
                {
                    ...
                    // 添加pes头  
                    Gb28181_make_pes_header(pBuff, szTempPacketHead.Length, stream_type == 1 ? 0xC0 : 0xE0, nSize,
                        (pPacker.s64CurPts / 1), (pPacker.s64CurPts / 3));


                    //最后在添加rtp头并发送数据  
                    if (Gb28181_send_rtp_pack(pBuff, startIndex, szTempPacketHead.Length + nSize + PES_HDR_LEN,
                        ((nSize == nFrameLen) ? 1 : 0), pPacker) != 0)
                    {
...
*/
        ...
        int Gb28181_send_rtp_pack(byte[] databuff, int index, int nDataLen, int mark_flag, Data_Info_s pPacker)
        {
            try
            {
                ...
                {
                    // 一帧数据发送完后，给mark标志位置1  
                    Gb28181_make_rtp_header(szRtpHdr, 0, ((mark_flag == 1) ? 1 : 0), ++pPacker.u16CSeq,
                        (pPacker.s64CurPts / 1), pPacker.u32Ssrc);

                    Array.Copy(szRtpHdr, pPacker.szBuff, RTP_HDR_LEN);
                    Array.Copy(databuff, index, pPacker.szBuff, RTP_HDR_LEN, nDataLen);
                    nRet = SendDataBuff(pPacker.szBuff, 0, RTP_HDR_LEN + nDataLen,
                        pPacker); //ZXB： 发送数据 为什么加databuff参数  需不需要加上index
                ...
                else
                {
                    ...
                    Gb28181_make_rtp_header(pPacker.szBuff, 0, 0, ++pPacker.u16CSeq, (pPacker.s64CurPts / 1),
                        pPacker.u32Ssrc);

                    //memcpy(pPacker->szBuff + RTP_HDR_LEN, databuff, nPlayLoadLen);
                    Array.Copy(databuff, index, pPacker.szBuff, RTP_HDR_LEN, nPlayLoadLen);

                    nRet = SendDataBuff(pPacker.szBuff, 0, RTP_HDR_LEN + nPlayLoadLen,
                        pPacker); //ZXB： 发送数据 为什么加databuff参数需不需要加上index
                    ...
                    while (nDataLen > 0)
                    {
                        ...
                        {
                            //一帧数据发送完，置mark标志位  
                            Gb28181_make_rtp_header(databuff, index, mark_flag, ++pPacker.u16CSeq,
                                (pPacker.s64CurPts / 1), pPacker.u32Ssrc);
                            nSendSize = nDataLen;
                        }
                        else
                        {
                            Gb28181_make_rtp_header(databuff, index, 0, ++pPacker.u16CSeq, (pPacker.s64CurPts / 1),
                                pPacker.u32Ssrc);
                            nSendSize = nPlayLoadLen;
                        }

                        nRet = SendDataBuff(databuff, index, RTP_HDR_LEN + nSendSize,
                            pPacker); //ZXB： 发送数据 为什么加databuff参数需不需要加上index
        ...
        int SendDataBuff(byte[] data, int startIndex, int size, Data_Info_s pPacker)
        {
            try
            {
                ...
                if (pPacker != null && pPacker.sendSOcket != null)
                {
                    ...
                    sendedCount =
                        pPacker.sendSOcket.SendTo(data, startIndex, size, SocketFlags.None, pPacker.remotePoint);
                    //if (sendedCount != 12)
        ...
        /// <returns></returns>
        private int Gb28181_make_rtp_header(byte[] pData, int startIndex, int marker_flag, ushort cseq, long curpts,
            uint ssrc)
        {
            try
            {
...
*/
      ...
      *@return:   0 success, others failed
     */
        private int Gb28181_make_pes_header(byte[] pData, int startIndex, int stream_id, int payload_len, ulong pts,
            ulong dts)
        {
            try
            {
