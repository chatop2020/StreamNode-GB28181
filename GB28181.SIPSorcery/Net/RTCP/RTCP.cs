...
namespace GB28181.Net
{
    public class RTPReceiveRecord
    {
        ...
        public bool
            InSequence =
                false; // Whether the packet is the next expected one in the sequence. Only in sequnce packets are used for sampling jitter.
        ...
        public RTPReceiveRecord(DateTime receiveTime, UInt16 sequenceNumber, long rtpBytes, uint jitter,
            bool inSequence, bool jitterDiscard)
        {
    ...
    public class RTCPReport
    {
        ...
        public uint
            LastReceivedReportNumber; // The report number of teh last RTCP report that was received from the remote agent.
        ...
        public byte[] GetBytes()
        {
            ...
            if (BitConverter.IsLittleEndian)
            {
                ...
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((ulong) SampleStartTime.Ticks)), 0,
                    payload, 4, 8);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((ulong) SampleEndTime.Ticks)), 0,
                    payload, 12, 8);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(StartSequenceNumber)), 0, payload, 20,
                    2);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian(EndSequenceNumber)), 0, payload, 22,
                    2);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((uint) TotalPackets)), 0, payload, 24,
                    4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((uint) OutOfOrder)), 0, payload, 28,
                    4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((uint) JitterAverage)), 0, payload,
                    32, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((uint) JitterMaximum)), 0, payload,
                    36, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((uint) JitterDiscards)), 0, payload,
                    40, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((uint) PacketsLost)), 0, payload, 44,
                    4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((uint) Duplicates)), 0, payload, 48,
                    4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((ulong) BytesReceived)), 0, payload,
                    52, 8);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((uint) TransmissionRate)), 0, payload,
                    60, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((ulong) Duration)), 0, payload, 64,
                    8);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((uint) AverageTransitTime)), 0,
                    payload, 72, 4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((uint) ReportNumber)), 0, payload, 76,
                    4);
                Buffer.BlockCopy(BitConverter.GetBytes(NetConvert.DoReverseEndian((uint) LastReceivedReportNumber)), 0,
                    payload, 80, 4);
            }
            else
            {
    ...
    public class RTCPReportSampler
    {
        ...
        private const string RTCP_FORMAT_STRING =
      ...
      //  private static ILog logger = LogManager.GetLogger("rtcp");
        ...
        private Dictionary<UInt16, RTPReceiveRecord>
            m_rcvdSeqNums =
                new Dictionary<UInt16, RTPReceiveRecord>(); // [<sequence number>, <RTCPMeasurement>] used to record received sequence numbers to detect duplicates.
        ...
        public void RecordRTPReceive(DateTime receiveTime, UInt16 sequenceNumber, long bytesReceived, uint jitter)
        {
            try
            {
                ...
                {
                    bool inSequence = (m_windowLastSeqNum != UInt16.MaxValue)
                        ? sequenceNumber == m_windowLastSeqNum + 1
                        : sequenceNumber == 0;
                    ...
                    */
                    ...
                    RTPReceiveRecord measurement = new RTPReceiveRecord(receiveTime, sequenceNumber, bytesReceived,
                        jitter, inSequence, jitterDiscard);
        ...
        private RTCPReport CheckForAvailableSample(UInt16 sequenceNumber)
        {
            try
            {
                ...
                if (DateTime.Now.Subtract(m_lastSampleTime).TotalMilliseconds > (4 * ReportSampleDuration))
                {
                    ...
                    int endSeqNum = (sequenceNumber < m_windowStartSeqNum)
                        ? sequenceNumber + UInt16.MaxValue + 1
                        : sequenceNumber;
                    ...
                    {
                        UInt16 testSeqNum = (seqNum > UInt16.MaxValue)
                            ? Convert.ToUInt16((seqNum % UInt16.MaxValue) - 1)
                            : Convert.ToUInt16(seqNum);
                ...
                if (sampleAvailable)
                {
                    ...
                    TimeSpan measurementsSampleDuration = m_rcvdSeqNums[endSampleSeqNumMinusOne].ReceiveTime
                        .Subtract(m_rcvdSeqNums[m_windowStartSeqNum].ReceiveTime);
        ...
        /// </summary>
        private RTCPReport Sample(UInt16 sampleStartSequenceNumber, UInt16 sampleEndSequenceNumber,
            TimeSpan sampleDuration)
        {
            try
            {
                ...
                int endSequence = (sampleEndSequenceNumber < sampleStartSequenceNumber)
                    ? sampleEndSequenceNumber + UInt16.MaxValue + 1
                    : sampleEndSequenceNumber;
                ...
                {
                    UInt16 testSeqNum = (index > UInt16.MaxValue)
                        ? Convert.ToUInt16((index % UInt16.MaxValue) - 1)
