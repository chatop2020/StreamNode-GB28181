...
namespace GB28181.Servers
{
    public class SIPRegistrarBindingsManager
    {
        private class NATKeepAliveJob
        {
            ...
            public NATKeepAliveJob(SIPEndPoint proxyEndPoint, SIPEndPoint remoteEndPoint, DateTime endTime,
                string owner)
            {
        ...
        private const int
            DEFAULT_BINDINGS_PER_USER =
                1; // The default maixmim number of bindings that will be allowed for each unique SIP account.

        private const int
            REMOVE_EXPIRED_BINDINGS_INTERVAL =
                3000; // The interval in seconds at which to check for and remove expired bindings.
                        ...
                        m_natKeepAliveJobs.Values.ToList().ForEach((job) =>
                        {
                            ...
                            try
                            {
                                ...
                                else if (job.NextSendTime == null || job.NextSendTime < DateTime.Now)
                                {
                                    ...
                                    FireSIPMonitorLogEvent(new SIPMonitorConsoleEvent(
                                        SIPMonitorServerTypesEnum.NATKeepAlive, SIPMonitorEventTypesEnum.NATKeepAlive,
                                        "Requesting NAT keep-alive from proxy socket " + job.ProxyEndPoint + " to " +
                                        job.RemoteEndPoint + ".", job.Owner));
