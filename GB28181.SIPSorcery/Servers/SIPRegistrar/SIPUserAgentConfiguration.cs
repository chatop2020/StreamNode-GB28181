...
namespace GB28181.Servers
{
    public class SIPUserAgentConfiguration
    {
        ...
        public int
            MaxAllowedExpiryTime =
                3600; // Overrules the default max expiry time the Registrar is using and allows specific user agents to have expirys up to this value.

        public bool
            ContactListSupported =
                true; // If false means the user agent wants only the Contact header it supplied returned in the Ok response.
    ...
    public class SIPUserAgentConfigurationManager
    {
       // private static ILog logger = AppState.logger;
        ...
        private Dictionary<string, SIPUserAgentConfiguration> ParseSIPUserAgentConfigurations(
            XmlNode userAgentConifgNode)
        {
            try
            {
                Dictionary<string, SIPUserAgentConfiguration> userAgentConfigs =
                    new Dictionary<string, SIPUserAgentConfiguration>();
                ...
                if (userAgentConifgNode != null && userAgentConifgNode.ChildNodes.Count != 0)
                {
                    foreach (XmlNode userAgentNode in userAgentConifgNode.SelectNodes("useragent"))
                    {
                        ...
                        bool contactListSupported = (userAgentNode.Attributes.GetNamedItem("contactlists") != null)
                            ? Convert.ToBoolean(userAgentNode.Attributes.GetNamedItem("contactlists").Value)
                            : true;
                        string userAgent = userAgentNode.Attributes.GetNamedItem("agent").Value;
                        SIPUserAgentConfiguration userAgentConfig =
                            new SIPUserAgentConfiguration(expiry, contactListSupported, userAgent);

                        if (userAgentConfig.UserAgentRegex != null &&
        ...
        public class SIPUserAgentConfigurationManagerUnitTest {
            ...
            public void GetExpiryUnitTest() {
                ...
                SIPUserAgentConfigurationManager uaConfigManager =
 new SIPUserAgentConfigurationManager(regExpiryDom.DocumentElement);
