...
namespace SIPSorcery.SIP.App
{
    public delegate bool
        SIPMonitorAuthenticationDelegate(string username,
            string password); // Delegate to authenticate connections to the SIP Monitor Server.

    public delegate void DialogueBridgeCreatedDelegate(SIPDialogue clientDialogue, SIPDialogue forwardedDialogue,
        string owner);
    ...
    public delegate void BlindTransferDelegate(SIPDialogue deadDialogue, SIPDialogue orphanedDialogue,
        SIPDialogue answeredDialogue);
    ...
    public delegate void SIPUASStateChangedDelegate(ISIPServerUserAgent uas, SIPResponseStatusCodesEnum statusCode,
        string reasonPhrase);
    ...
    public delegate List<SIPAccount> GetSIPAccountsForUserDelegate(string username, string domain, int offset,
        int limit);

    public delegate List<SIPAccount> GetSIPAccountsForOwnerDelegate(string owner, int offset, int limit);

    // Authorisation delegates.
    public delegate SIPRequestAuthenticationResult SIPAuthenticateRequestDelegate(SIPEndPoint localSIPEndPoint,
        SIPEndPoint remoteEndPoint, SIPRequest sipRequest, SIPAccount sipAccount, SIPMonitorLogDelegate log);
