...
namespace SIPSorcery.SIP.App
{
    ...
    {
        public const string
            SERIALISATION_PREFIX = "2"; // Prefix appended to the front of a serialised event to identify the type. 

        public SIPMonitorMachineEventTypesEnum MachineEventType;

        public string
            ResourceID; // For a dialog SIP Event this will be the dialogue ID, for a presence SIP event this will be the SIP Account ID.
        ...
        public SIPMonitorMachineEvent(SIPMonitorMachineEventTypesEnum machineEventType, string owner,
            SIPEndPoint remoteEndPoint, string message)
        {
            m_serialisationPrefix = SERIALISATION_PREFIX;
        ...
        public SIPMonitorMachineEvent(SIPMonitorMachineEventTypesEnum machineEventType, string owner, string resourceID,
            SIPURI resourceURI)
        {
            m_serialisationPrefix = SERIALISATION_PREFIX;
        ...
        public static SIPMonitorMachineEvent ParseMachineEventCSV(string eventCSV)
        {
            try
            {
                ...
                machineEvent.MachineEventType =
                    SIPMonitorMachineEventTypes.GetMonitorMachineTypeForId(Convert.ToInt32(eventFields[3]));
                machineEvent.Created = DateTimeOffset.ParseExact(eventFields[4], SERIALISATION_DATETIME_FORMAT,
        ...
        public string ToAnonymousCSV()
        {
            try
            {
                ...
                if (RemoteEndPoint != null)
                {
                    ...
                    remoteSocket = (RemoteEndPoint != null)
                        ? anonymisedIPAddress.ToString() + ":" + RemoteEndPoint.Port
                        : null;
                }
