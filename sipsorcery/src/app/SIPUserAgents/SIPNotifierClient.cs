    ...
    public class SIPNotifierClient<T> where T : SIPEvent, new()
    {
        ...
        public void Stop()
        {
            try
            {
                if (!m_exit)
                {
                    logger.LogDebug("Stopping SIP notifier user agent for user " + m_authUsername +
                                    " and resource URI " + m_resourceURI.ToString() + ".");
        ...
        private void StartSubscription()
        {
            try
            {
                logger.LogDebug("SIPNotifierClient starting for " + m_resourceURI.ToString() + " and event package " +
                ...
                while (!m_exit)
                {
                    ...
                    if (!m_exit)
                    {
                        ...
                        if (m_subscribed)
                        {
                            ...
                            Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.NotifierClient,
                                SIPMonitorEventTypesEnum.SubscribeFailed,
                                "Rescheduling next attempt for a successful subscription to " +
                                m_resourceURI.ToString() + " in " + (m_expiry - RESCHEDULE_SUBSCRIBE_MARGIN) + "s.",
                                null));
                ...
                Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.NotifierClient,
                    SIPMonitorEventTypesEnum.SubscribeFailed,
                    "Subscription attempts to " + m_resourceURI.ToString() + " for " + m_sipEventPackage.ToString() +
                    " have been halted.", null));
            }
            catch (Exception excp)
            {
            ...
            SIPURI contactURI)
        {
            try
            {
                if (m_attempts >= MAX_SUBSCRIBE_ATTEMPTS)
                {
                    Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.NotifierClient,
                        SIPMonitorEventTypesEnum.SubscribeFailed,
                        "Subscription to " + subscribeURI.ToString() +
        ...
        private void SubsribeTransactionTimedOut(SIPTransaction sipTransaction)
        {
            ...
            {
                SubscriptionFailed(m_resourceURI, SIPResponseStatusCodesEnum.ServerTimeout,
                    "Subscription request to " + m_resourceURI.ToString() + " timed out.");
            }
            ...
            SIPEndPoint remoteEndPoint, SIPTransaction sipTransaction, SIPResponse sipResponse)
        {
            try
            {
                if (sipResponse.Status == SIPResponseStatusCodesEnum.IntervalTooBrief)
                {
                    ...
                    Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.NotifierClient,
                        SIPMonitorEventTypesEnum.SubscribeFailed,
                        "A subscribe request was rejected with IntervalTooBrief, adjusting expiry to " + m_expiry +
                        " and trying again.", null));
                    Subscribe(m_resourceURI, m_expiry, m_sipEventPackage, m_subscribeCallID, null);
                }
                         ...
                         sipResponse.Status == SIPResponseStatusCodesEnum.Unauthorised)
                {
                    ...
                    else if (sipResponse.Header.AuthenticationHeader != null)
                    {
                        ...
                        {
                            Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.NotifierClient,
                                SIPMonitorEventTypesEnum.SubscribeAuth,
                                "Attempting authentication for subscribe request for event package " +
                                m_sipEventPackage.ToString() + " and " + m_resourceURI.ToString() + ".", null));
                ...
                else if (sipResponse.StatusCode >= 200 && sipResponse.StatusCode <= 299)
                {
                    Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.NotifierClient,
                        SIPMonitorEventTypesEnum.SubscribeAuth,
                        "Authenticating subscribe request for event package " + m_sipEventPackage.ToString() + " and " +
                        m_resourceURI.ToString() + " was successful.", null));
