    ...
    public class SIPServerUserAgent : ISIPServerUserAgent
    {
        ...
        public void SetTraceDelegate(SIPTransactionTraceMessageDelegate traceDelegate)
        {
            ...
            traceDelegate(m_uasTransaction, SIPMonitorEventTypesEnum.SIPTransaction + "=>" + "Request received " +
                                            m_uasTransaction.TransactionRequest.LocalSIPEndPoint +
                    ...
                    if (m_sipAccount == null)
                    {
                        ...
                        if (m_sipAccount == null)
                        {
                            Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.AppServer,
                                ...
                                "Rejecting public call for " + m_sipUsername + "@" + m_sipDomain +
            ...
            SIPDialogueTransferModesEnum transferMode, string[] customHeaders)
        {
            try
            {
                ...
                if (m_uasTransaction.TransactionFinalResponse != null)
                {
                    Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.AppServer,
                        SIPMonitorEventTypesEnum.DialPlan,
                        "UAS Answer was called on an already answered call, ignoring.", m_owner));
                    return null;
                }
                else
                {
            ...
            try
            {
                if (m_uasTransaction.TransactionFinalResponse == null)
                {
                    if ((int) failureStatus < 400)
                    {
                        Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.AppServer,
                            SIPMonitorEventTypesEnum.DialPlan,
                            "UAS Reject was passed an invalid response status of " + (int) failureStatus +
                            ", ignoring.", m_owner));
                    }
                    else
                    {
                        ...
                        Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.AppServer,
                            SIPMonitorEventTypesEnum.DialPlan,
                            "UAS call failed with a response status of " + (int) failureStatus + failureReason + ".",
                            m_owner));
        ...
        private void ClientTimedOut(SIPTransaction sipTransaction)
        {
            try
            {
                ...
                Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.UserAgentServer,
                    SIPMonitorEventTypesEnum.DialPlan,
                    "UAS for " + m_uasTransaction.TransactionRequest.URI.ToString() +
                    " timed out in transaction state " + m_uasTransaction.TransactionState + ".", null));
