...
namespace SIPSorcery.SIP.App
{
    public class SIPRegistrationUserAgent
    {
            ...
            int maxRegisterAttempts = DEFAULT_MAX_REGISTER_ATTEMPTS)
        {
            ...
            Log_External = (ev) => Logger.Logger.Debug(ev?.Message);
        }
        ...
        public void Start()
        {
            ...
            Logger.Logger.Debug(
                $"Starting SIPRegistrationUserAgent for {m_sipAccountAOR.ToString()}, callback period {callbackPeriod}ms.");
        ...
        private void DoRegistration(object state)
        {
            if (Monitor.TryEnter(m_waitForRegistrationMRE))
            {
                try
                {
                    Logger.Logger.Debug("DoRegistration for " + m_sipAccountAOR.ToString() + ".");
                ...
                {
                    Logger.Logger.Error("Exception DoRegistration Start. ->" + excp.Message);
                }
                finally
                {
        ...
        public void SetExpiry(int expiry)
        {
            ...
            {
                Logger.Logger.Info(
                    $"Expiry for registration agent for {m_sipAccountAOR.ToString()} updated from {m_expiry} to {newExpiry}.");
        ...
        private void SendInitialRegister()
        {
            try
            {
                ...
                else
                {
                    ...
                    if (registrarSIPEndPoint == null)
                    {
                        Logger.Logger.Warn("SIPRegistrationAgent could not resolve " + m_registrarHost + ".");
        ...
        private void RegistrationTimedOut(SIPTransaction sipTransaction)
        {
            m_isRegistered = false;
            RegistrationTemporaryFailure?.Invoke(m_sipAccountAOR,
                "Registration transaction to " + m_registrarHost + " for " + m_sipAccountAOR.ToString() +
                " timed out.");
            m_waitForRegistrationMRE.Set();
        }
            ...
            SIPTransaction sipTransaction, SIPResponse sipResponse)
        {
            try
            {
                Log_External(new SIPMonitorConsoleEvent(SIPMonitorServerTypesEnum.UserAgentClient,
                    SIPMonitorEventTypesEnum.ContactRegisterInProgress,
                    "Server auth response " + sipResponse.Status + " received for " + m_sipAccountAOR.ToString() + ".",
                    null));
        ...
        private SIPRequest GetAuthenticatedRegistrationRequest(SIPRequest registerRequest, SIPResponse sipResponse)
        {
            try
            {
                ...
                authRequest.SetCredentials(username, m_password, registerRequest.URI.ToString(),
