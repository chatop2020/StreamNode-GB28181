...
namespace SIPSorcery.Sys
{
    ...
    public class NetServices
    {
        ...
        private const int
            MAXIMUM_UDP_PORT_BIND_ATTEMPTS =
                25; // The maximum number of re-attempts that will be made when trying to bind a UDP socket.

        private const string
            INTERNET_IPADDRESS =
                "8.8.8.8"; // IP address to use when getting default IP address from OS. No connection is established.

        private const int
            NETWORK_TEST_PORT =
                5060; // Port to use when doing a Udp.Connect to determine local IP address (port 0 does not work on MacOS).

        private const int
            LOCAL_ADDRESS_CACHE_LIFETIME_SECONDS =
                300; // The amount of time to leave the result of a local IP address determination in the cache.
            ...
            bool useDualMode = true)
        {
            if (requireEvenPort && port != 0)
            {
                throw new ArgumentException(
                    "Cannot specify both require even port and a specific port to bind on. Set port to 0.");
            }
            ...
            Logger.Logger.Debug($"CreateBoundUdpSocket attempting to create and bind UDP socket(s) on {logEp}.");
            ...
            while (bindAttempts < MAXIMUM_UDP_PORT_BIND_ATTEMPTS)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Logger.Logger.Debug(
                                    $"CreateBoundUdpSocket successfully bound on {socket.LocalEndPoint}.");
                            }
                        }
                ...
                catch (SocketException sockExcp)
                {
                    ...
                    {
                        Logger.Logger.Error($"SocketException in NetServices.CreateCheckedUdpSocket. ->{sockExcp}");
                        throw;
                    }
                }
        ...
        private static void BindUdpSocket(Socket socket, IPAddress bindAddress, int port)
        {
                ...
                RuntimeInformation.OSDescription.Contains("Microsoft"))
            {
                ...
                {
                    Logger.Logger.Debug($"WSL detected, carrying out bind check on 0.0.0.0:{port}.");
        ...
        public static void CreateRtpSocket(bool createControlSocket, IPAddress bindAddress, out Socket rtpSocket,
            out Socket controlSocket)
        {
            ...
            Logger.Logger.Debug($"CreateRtpSocket attempting to create and bind RTP socket(s) on {bindEP}.");
            ...
            else if (!createControlSocket && rtpSocket != null)
            {
                ...
                {
                    Logger.Logger.Debug($"Successfully bound RTP socket {rtpSocket.LocalEndPoint}.");
                }
            }
            else
            {
