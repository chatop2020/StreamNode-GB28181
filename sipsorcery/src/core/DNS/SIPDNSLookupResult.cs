...
namespace SIPSorcery.SIP
{
    public class SIPDNSServiceResult
    {
        ...
        public int
            Priority; // The priority value assigned to the NAPTR/SRV record. A client MUST attempt to contact the target host with the lowest-numbered priority it can

        // reach; target hosts with the same priority SHOULD be tried in an order defined by the weight field.
        public int
            Weight; // The weight value assigned to the SRV record. Larger weights SHOULD be given a proportionately higher probability of being selected.
        ...
        public readonly DateTime
            ValidUntil; // rj2: Time when DNS-Result becomes invalid, ResolveTime + TimeToLive-Seconds, Member for easy comparison

        public SIPDNSServiceResult(SIPServicesEnum sipService, int priority, int weight, uint ttl, string data,
            int port, DateTime resolvedAt)
        {
    ...
    public class SIPDNSLookupResult
    {
        ...
        public bool
            Pending; // If an async lookup request is made this will be set to true if no immediate result is available.
        ...
        public SIPDNSLookupResult(SIPURI uri, SIPEndPoint resultEndPoint)
        {
            ...
            EndPointResults = new List<SIPDNSLookupEndPoint>()
                {new SIPDNSLookupEndPoint(resultEndPoint, Int32.MaxValue)};
        }
        ...
        public SIPDNSServiceResult GetNextUnusedSRV()
        {
            ...
            {
                return (from srv in SIPSRVResults where srv.EndPointsResolvedAt == null orderby srv.Priority select srv)
                    .FirstOrDefault();
            }
