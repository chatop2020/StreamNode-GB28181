...
namespace Heijden.DNS
{
    ...
    public class Resolver
    {
        ...
        private const int
            SWITCH_ACTIVE_TIMEOUT_COUNT =
                5; // The timeout in seconds that if no response is received on the active DNS server it will be switched.

        private const int FAILURE_RETRY = 60; // The timeout in seconds to retry DNS lookups that failed.

        private const int
            MIN_CACHE_SECONDS =
                60; // A record will not be removed from the cache when it has been in less than this number of seconds irrespective of the DNS TTL.
        ...
        private DNSResponse SearchInCache(Question question)
        {
            ...
            foreach (RR rr in response.RecordsRR)
            {
                ...
                {
                    logger.LogDebug(
                        $"DNS cache out of date result found for {strKey}. SecondsLived({secondsLived}) is >= remaining TimeToLive({rr.TTL})");
        ...
        private DNSResponse UdpRequest(DNSRequest request, List<IPEndPoint> dnsServers, int timeout)
        {
            ...
            for (int nAttempts = 0; nAttempts < m_Retries && !m_stop; nAttempts++)
            {
                for (int nDnsServer = 0; nDnsServer < dnsServers.Count && !m_stop; nDnsServer++)
                {
                    ...
                    {
                        logger.LogDebug("Resolver not sending UDP DNS request to " + dnsServers[nDnsServer] +
                    ...
                    try
                    {
                        ...
                        {
                            logger.LogDebug("Error " + response.header.RCODE + " in Resolver UdpRequest " +
                                            requestStrWithDns);
                        }
                    }
        ...
        private DNSResponse TcpRequest(DNSRequest request, List<IPEndPoint> dnsServers, int timeout)
        {
            ...
            for (int nAttempts = 0; nAttempts < m_Retries && !m_stop; nAttempts++)
            {
                for (int nDnsServer = 0; nDnsServer < dnsServers.Count && !m_stop; nDnsServer++)
                {
                    ...
                    if (GetTimeoutCount(dnsServers[nDnsServer]) >= SWITCH_ACTIVE_TIMEOUT_COUNT)
                    {
                        logger.LogDebug("Resolver not sending TCP DNS request to " + dnsServers[nDnsServer] +
                                        " because of maximum count of request timeouts reached");
                        continue;
                    }
                    ...
                    try
                    {
                        ...
                        while (true)
                        {
                            ...
                            if (response.header.RCODE != RCode.NOERROR)
                            {
                                logger.LogDebug("Error " + response.header.RCODE + " in Resolver TcpRequest " +
                                                requestStrWithDns);
                                return response;
                            }
                            else
        ...
        private IPEndPoint GetActiveDNSServer()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (m_receiveTimeouts.ContainsKey(m_activeDNSServer) &&
                        m_receiveTimeouts[m_activeDNSServer] >= SWITCH_ACTIVE_TIMEOUT_COUNT)
                    {
                        ...
                        foreach (IPEndPoint dnsServer in m_DnsServers)
                        {
                            ...
                            {
                                logger.LogDebug("Switching active DNS server from " + m_activeDNSServer + " to " +
                                                dnsServer + ".");
